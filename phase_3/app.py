...
with col_right:
    ...
    if submit and user_input:
        ...
        try:
            ...
            # Bypassing and calling QAPipeline directly if server is not running
            try:
                from qa_pipeline import QAPipeline
                qa = QAPipeline()
                res = qa.ask(user_input)
                answer = res.get("result", "No response content.")
            except Exception as e:
                answer = f"Error: Cannot connect to FastAPI server, direct call failed: {str(e)}"
